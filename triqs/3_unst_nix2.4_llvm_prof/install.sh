...
MODULES="modules/2.4 gcc flexiblas openmpi cmake gmp fftw nfft hdf5/mpi boost python/3.12 python-mpi/3.12 intel-oneapi-mkl llvm/19 eigen mpfr gperftools"
module purge
module load ${MODULES}
...
BUILDINFO=3_unst_nix2.4_llvm_prof
...
(
    ...
    cmake -S triqs.src -B triqs.build -DCMAKE_INSTALL_PREFIX=${INSTALLDIR} -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S cthyb.src -B cthyb.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S ctint.src -B ctint.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S ctseg.src -B ctseg.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S w2dyn.src -B w2dyn.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S dft_tools.src -B dft_tools.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S maxent.src -B maxent.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S Nevanlinna.src -B Nevanlinna.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S tprf.src -B tprf.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S hubbardI.src -B hubbardI.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S hartree_fock.src -B hartree_fock.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S forktps.src -B forktps.build -DBLAS_LIBRARIES="-L${FLEXIBLAS_ROOT}/libi64 -lflexiblas -lpthread" -Dgpu_backend=none -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S solid_dmft.src -B solid_dmft.build -DMPIEXEC_MAX_NUMPROCS=4 -DTest_GW_embedding=OFF -DCMAKE_BUILD_TYPE=RelWithDebInfo
    ...
    cmake -S modest.src -B modest.build -DCMAKE_BUILD_TYPE=RelWithDebInfo
