...
BUILDINFO=3_unst_nix2_llvm_san
...
(
    ...
    cmake ../triqs.src -DCMAKE_INSTALL_PREFIX=${INSTALLDIR} -DBuild_Deps=Always -DCMAKE_BUILD_TYPE=RelWithDebInfo -DASAN=ON -DUBSAN=ON
    # make / test / install
    make -j$NCORES
    make install
    #################
    ...
    cmake ../cthyb.src -DCMAKE_BUILD_TYPE=RelWithDebInfo -DASAN=ON -DUBSAN=ON
    # make / test / install
    make -j$NCORES
    make install
    #################
    ...
    cmake ../dft_tools.src -DCMAKE_BUILD_TYPE=RelWithDebInfo -DASAN=ON -DUBSAN=ON
    # make / test / install
    make -j$NCORES
    ctest -j$NCORES
    make install
