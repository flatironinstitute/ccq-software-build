...
#
# load modules
MODULES="modules/2.2-20230808 gcc/11.4.0 openmpi/4 fftw intel-oneapi-mkl hdf5/mpi git wannier90/dev_nix2.2_gnu_ompi"
module purge
module load ${MODULES}

MODULEDIR=$(git rev-parse --show-toplevel)/modules
BUILDINFO=6.4.2_nix2.2_gnu
...
(
    ...
    cp ${WANNIER90_ROOT}/lib/libwannier_seq.a ${BUILDDIR}/${VASPFILE}/libwannier.a

    # patch for Vasp CSC
    cd src
    rsync -av ${INSTALLDIR}/vasp_diffs ${BUILDDIR}/${VASPFILE}
    for name in electron.F fileio.F locproj.F mlwf.F; do patch $name -p1 -i ../vasp_diffs/$name; done
